...
LineReader::LineReader(hdfsFS& fs, hdfsFile& handle)
    ...
    file_end_ = false;
    fill();
    line = reinterpret_cast<char*>(malloc(LINE_DEFAULT_SIZE * sizeof(char)));
}
...
void LineReader::double_linebuf() {
    size *= 2;
    line = reinterpret_cast<char*>(realloc(line, size * sizeof(char)));
}
...
void LineReader::append_line() {
    if (buf_pos_ == buf_size_)
        return;
    char* pch = reinterpret_cast<char*>(memchr(buf_ + buf_pos_, '\n', buf_size_ - buf_pos_));
    if (pch == NULL) {
        ...
        if (!file_end_)
            ...
            return;
        pch = reinterpret_cast<char*>(memchr(buf_, '\n', buf_size_));
        while (pch == NULL) {
...
void LineWriter::next_hdl() {
    ...
    char buffer[10];
    if (me_ >= 0) {
        snprintf(buffer, sizeof(buffer), "%d", me_);
        strcat(fname, buffer);
        strcat(fname, "_");
    }
    snprintf(buffer, sizeof(buffer), "%d", nxt_part_);
    strcat(fname, buffer);
    // flush old file
    if (nxt_part_ > 0) {
        if (hdfsFlush(fs_, cur_hdl_)) {
...
void LineWriter::write_line(char* line, int num) {
    // +1 because of '\n'
    if (cur_size_ + num + 1 > HDFS_BLOCK_SIZE) {
        next_hdl();
    }
...
void BufferedWriter::next_hdl() {
    ...
    if (me_ >= 0) {
        snprintf(fname, sizeof(fname), "part_%d_%d", me_, next_part_);
    } else {
        snprintf(fname, sizeof(fname), "part_%d", next_part_);
    }
    ...
    int path_len = strlen(path_) + strlen(fname) + 2;
    char* file_path = new char[path_len];
    snprintf(file_path, path_len, "%s/%s", path_, fname);
    cur_hdl_ = get_w_handle(file_path, fs_);
    delete[] file_path;
}
...
void BufferedWriter::check() {
    if (buf_.size() >= HDFS_BLOCK_SIZE) {
        ...
        // -1 means "output in the specified file only"
        if (me_ != -1) {
            next_hdl();
        }
    }
