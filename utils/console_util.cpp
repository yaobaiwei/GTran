...
void ConsoleUtil::LoadHistory() {
    ...
    memcpy(reinterpret_cast<void*>(buffer_), reinterpret_cast<void*>(line_bfs_[actual_line_no]), BUFFER_SIZE);
...
void ConsoleUtil::OnKeyUp() {
    ...
    if (cur_roll_no_ == 0) {
        ...
        memcpy(reinterpret_cast<void*>(line_bfs_[cur_line_ptr_no_]), reinterpret_cast<void*>(buffer_), BUFFER_SIZE);
...
void ConsoleUtil::OnPrintableKey(int key) {
    ...
    if (cur_line_pos_ == cur_line_len_) {
        ...
        printf("%c", key);
        snprintf(&buffer_[cur_line_len_++], sizeof(buffer_), "%c", key);
        cur_line_pos_++;
    } else {
        // clear the line, print the content, then move back the cursor
        int len_to_backup = cur_line_len_ - cur_line_pos_;
        memcpy(reinterpret_cast<void*>(tmp_buffer_),
            reinterpret_cast<void*>(&buffer_[cur_line_pos_]),
            len_to_backup);
        buffer_[cur_line_pos_] = key;
        memcpy(reinterpret_cast<void*>(&buffer_[cur_line_pos_ + 1]),
            reinterpret_cast<void*>(tmp_buffer_),
            len_to_backup);
        SimpleRefreshLine();
...
void ConsoleUtil::OnKeyDelete() {
    ...
    if (cur_line_pos_ == cur_line_len_ - 1) {
        ...
        memcpy(reinterpret_cast<void*>(tmp_buffer_),
            reinterpret_cast<void*>(&buffer_[cur_line_pos_ + 1]),
            len_to_backup);
        memcpy(reinterpret_cast<void*>(&buffer_[cur_line_pos_]),
            reinterpret_cast<void*>(tmp_buffer_),
            len_to_backup);
        buffer_[cur_line_len_ - 1] = 0;
        SimpleRefreshLine();
...
string ConsoleUtil::TryConsoleInput(string line_head) {
    ...
    while (true) {
        ...
        if (isprint(key)) {
            ...
            } else if (key == KEY_BACKSPACE || key == KEY_DIRECT_BACKSPACE) {
                // delete to the left of current cursor
                OnKeyBackspace();
...
void ConsoleUtil::WriteConsoleHistory(string path) {
    ...
    for (int i = 0; i < line_to_write; i++) {
        ...
        of << string(line_bfs_[(cur_line_no_ - fake_iter - 1) % BUFFER_LINE]) << std::endl;
    }
