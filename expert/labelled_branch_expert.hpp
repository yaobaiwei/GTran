...
class LabelledBranchExpertBase :  public AbstractExpert {
 ...
 public:
    LabelledBranchExpertBase(int id,
            int num_thread,
            AbstractMailbox* mailbox,
            CoreAffinity* core_affinity,
            msg_id_alloc* allocator) :
    ...
    void process(const QueryPlan & qplan,  Message & msg) {
        ...
        if (msg.meta.msg_type == MSG_T::SPAWN) {
            ...
            send_branch_msg(tid, qplan.experts, msg, msg_id);
            ac->second.branch_counter = make_pair(get_steps_count(qplan.experts[msg.meta.step]), 0);
            ...
            process_branch(tid, qplan.experts, msg, ac, isReady);
    ...
    void clean_trx_data(uint64_t trxid) override {
        ...
        if (trx_table_.find(ac, trxid)) {
            // Erase tmp data of not finished labelled_branch expert
 ...
 protected:
    ...
    virtual void process_branch(int tid,
            const vector<Expert_Object> & experts,
            Message & msg,
            typename BranchDataTable::accessor& ac,
            bool isReady) = 0;
 ...
 private:
    ...
    void send_branch_msg(int tid, const vector<Expert_Object> & experts, Message & msg, uint64_t msg_id) {
        ...
        msg.CreateBranchedMsgWithHisLabel(experts, step_vec, msg_id, num_thread_, core_affinity_, msg_vec);
...
class BranchFilterExpert : public LabelledBranchExpertBase<BranchData::branch_filter_data> {
 public:
    BranchFilterExpert(int id,
            int num_thread,
            AbstractMailbox* mailbox,
            CoreAffinity* core_affinity,
            msg_id_alloc* allocator) :
        LabelledBranchExpertBase<BranchData::branch_filter_data>(id, num_thread, mailbox, core_affinity, allocator) {}
 ...
 private:
    ...
    void process_branch(int tid,
            const vector<Expert_Object> & experts,
            Message & msg,
            BranchDataTable::accessor& ac,
            bool isReady) {
        ...
        if (isReady) {
            ...
            for (auto& pair : data) {
                pair.second.erase(
                        remove_if(pair.second.begin(), pair.second.end(), checkFunction), pair.second.end());
            }

            // remove last branch info
            msg.meta.branch_infos.pop_back();
            vector<Message> v;
            msg.CreateNextMsg(experts, data, num_thread_, core_affinity_, v);
            for (auto& m : v) {
                mailbox_->Send(tid, m);
            }
        }
