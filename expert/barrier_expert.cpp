...
#include "barrier_expert.hpp"

void EndExpert::do_work(int tid, const QueryPlan & qplan, Message & msg,
    ...
    if (msg.meta.msg_type == MSG_T::ABORT) {
        if (!ac->second.is_abort) {
            ...
            rc_->InsertAbortResult(msg.meta.qid, data);
        }
    } else {
...
void AggregateExpert::do_work(int tid, const QueryPlan & qplan, Message & msg,
    BarrierDataTable::accessor& ac, bool isReady) {
    ...
    if (isReady) {
        const Expert_Object& expert = qplan.experts[msg.meta.step];
        CHECK(expert.params.size() == 1);
        ...
        if (is_next_barrier(qplan.experts, msg.meta.step)) {
            ...
            msg.CreateNextMsg(qplan.experts, msg_data, num_thread_, core_affinity_, v);
        }
...
void CapExpert::do_work(int tid, const QueryPlan & qplan, Message & msg,
        BarrierDataTable::accessor& ac, bool isReady) {
    // all msg are collected
    if (isReady) {
        const Expert_Object& expert = qplan.experts[msg.meta.step];
...
void CountExpert::do_work(int tid, const QueryPlan & qplan, Message & msg,
        BarrierDataTable::accessor& ac, bool isReady) {
...
void DedupExpert::do_work(int tid, const QueryPlan & qplan, Message & msg,
        BarrierDataTable::accessor& ac, bool isReady) {
...
void GroupExpert::do_work(int tid, const QueryPlan & qplan, Message & msg,
        BarrierDataTable::accessor& ac, bool isReady) {
    auto& data_map = ac->second.data_map;
    int branch_key = get_branch_key(msg.meta);
    ...
    // get expert params
    const Expert_Object& expert = qplan.experts[msg.meta.step];
    CHECK(expert.params.size() == 2);
    int label_step = Tool::value_t2int(expert.params[1]);
...
void OrderExpert::do_work(int tid, const QueryPlan & qplan, Message & msg,
        BarrierDataTable::accessor& ac, bool isReady) {
    auto& data_map = ac->second.data_map;
    ...
    const Expert_Object& expert = qplan.experts[msg.meta.step];
    CHECK(expert.params.size() == 2);
    int label_step = Tool::value_t2int(expert.params[0]);
    ...
    // all msg are collected
    if (isReady) {
        Order_T order = (Order_T)Tool::value_t2int(expert.params[1]);
        vector<pair<history_t, vector<value_t>>> msg_data;
        if (label_step < 0) {
            for (auto& p : data_set) {
...
void PostValidationExpert::do_work(int tid, const QueryPlan & qplan, Message & msg,
        BarrierDataTable::accessor& ac, bool isReady) {
    if (msg.meta.msg_type == MSG_T::ABORT) {
...
void RangeExpert::do_work(int tid, const QueryPlan & qplan, Message & msg,
        BarrierDataTable::accessor& ac, bool isReady) {
    auto& counter_map = ac->second.counter_map;
    int branch_key = get_branch_key(msg.meta);
...
void CoinExpert::do_work(int tid, const QueryPlan & qplan, Message & msg,
        BarrierDataTable::accessor& ac, bool isReady) {
    auto& counter_map = ac->second.counter_map;
    int branch_key = get_branch_key(msg.meta);
...
void MathExpert::do_work(int tid, const QueryPlan & qplan, Message & msg,
        BarrierDataTable::accessor& ac, bool isReady) {
    auto& data_map = ac->second.data_map;
    int branch_key = get_branch_key(msg.meta);
...
void MathExpert::sum(BarrierData::math_meta_t& data, value_t& v) {
    data.count++;
    if (data.count == 1) {
        data.value = move(v);
        return;
...
void MathExpert::max(BarrierData::math_meta_t& data, value_t& v) {
...
void MathExpert::min(BarrierData::math_meta_t& data, value_t& v) {
    if (data.count == 0 || data.value > v) {
        data.value = move(v);
    }
    data.count++;
...
void MathExpert::to_double(BarrierData::math_meta_t& data, bool isMean) {
    value_t temp = data.value;
