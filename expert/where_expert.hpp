...
#ifndef EXPERT_WHERE_EXPERT_HPP_
#define EXPERT_WHERE_EXPERT_HPP_
...
class WhereExpert : public AbstractExpert {
 public:
    ...
    void process(const QueryPlan & qplan, Message & msg) {
        ...
        // Get Params
        CHECK(expert_obj.params.size() > 0 && (expert_obj.params.size() % 3) == 0);
 ...
 private:
        ...
        for (auto & pred : pred_chain) {
            ...
            if (step_labels.at(0) == -1) {
                ...
                for (auto & data_pair : data) {
                    ...
                    if (!GetHistoryValue(data_pair.first,
                                         vector<int>(step_labels.begin() + 1, step_labels.end()),
                                         his_val)) {
    ...
    void EvaluateForAggregate(
            vector<pair<history_t, vector<value_t>>> & data,
            map<value_t, int> & agg_data, int his_label, Predicate_T pred_type) {
...
#endif  // EXPERT_WHERE_EXPERT_HPP_
