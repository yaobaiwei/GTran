...
// p : Primitive
void PrimitiveRCTTable::GetRecentActionSet(Primitive_T p, const vector<uint64_t> & trxIDList, unordered_map<uint64_t, vector<rct_read_data_t>> & trx_rct_map) {
    for (auto & trxID : trxIDList) {
        rct_const_accessor rctca;
        vector<rct_read_data_t> rct_content;
