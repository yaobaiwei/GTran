...
private:
    ...
    Config * config_;
    Buffer * buf_;  //TODO(entityless): Discuss if it is necessary to use Buffer
...
public:
    MVCCKVStore(bool is_vp);  // if is_vp, then this instance VKVStore; or this instance is EKVStore.
    ptr_t Insert(const MVCCHeader& key, value_t& value); //TODO: replace to ptr_t
    value_t Get(ptr_t);
};
