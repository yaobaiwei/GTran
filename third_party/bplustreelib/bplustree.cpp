 ...
 * source: https://github.com/begeekmyfriend/bplustree/tree/in-memory
 */
...
// search target in array arr~arr+len-1
// return the index
static int key_binary_search(bp_key_t *arr, int len, bp_key_t target) {
    int low = -1;
    int high = len;
    while (low + 1 < high) {
...
static long long bplus_tree_search(struct bplus_tree *tree, bp_key_t key) {
    long long i, ret = -1;
...
static int non_leaf_split_right1(struct bplus_non_leaf *node,
                                 struct bplus_non_leaf *right,
                                 struct bplus_node *l_ch,
                                 struct bplus_node *r_ch, bp_key_t key,
                                 int insert) {
    int i, j, order = node->children;
    bp_key_t split_key;
    /* split = [m/2] */
...
static int non_leaf_insert(struct bplus_tree *tree, struct bplus_non_leaf *node,
    ...
    if (node->children == tree->order) {
        ...
        if (insert < split) {
            split_key =
                non_leaf_split_left(node, sibling, l_ch, r_ch, key, insert);
        } else if (insert == split) {
            split_key =
                non_leaf_split_right1(node, sibling, l_ch, r_ch, key, insert);
        } else {
            split_key =
                non_leaf_split_right2(node, sibling, l_ch, r_ch, key, insert);
        }
        /* build new parent */
        if (insert < split) {
            return parent_node_build(tree, (struct bplus_node *)sibling,
                                     (struct bplus_node *)node, split_key,
                                     level);
        } else {
            return parent_node_build(tree, (struct bplus_node *)node,
                                     (struct bplus_node *)sibling, split_key,
                                     level);
        }
    } else {
...
static void leaf_split_left(struct bplus_leaf *leaf, struct bplus_leaf *left,
                            bp_key_t key, long long data, int insert) {
    int i, j;
    /* split = [m/2] */
...
static void leaf_simple_insert(struct bplus_leaf *leaf, bp_key_t key,
                               long long data, int insert) {
    int i;
...
static int leaf_insert(struct bplus_tree *tree, struct bplus_leaf *leaf,
                       bp_key_t key, long long data) {
    /* search key location */
...
static int bplus_tree_insert(struct bplus_tree *tree, bp_key_t key,
                             long long data) {
    struct bplus_node *node = tree->root;
    while (node != NULL) {
        if (is_leaf(node)) {
...
static int non_leaf_sibling_select(struct bplus_non_leaf *l_sib,
...
static void non_leaf_remove(struct bplus_tree *tree,
                            struct bplus_non_leaf *node, int remove) {
    if (node->children <= (tree->order + 1) / 2) {
        ...
        if (parent != NULL) {
            ...
            if (non_leaf_sibling_select(l_sib, r_sib, parent, i) ==
                LEFT_SIBLING) {
...
static int leaf_sibling_select(struct bplus_leaf *l_sib,
...
static void leaf_shift_from_left(struct bplus_leaf *leaf,
                                 struct bplus_leaf *left, int parent_key_index,
                                 int remove) {
...
static void leaf_merge_into_left(struct bplus_leaf *leaf,
                                 struct bplus_leaf *left, int remove) {
    int i, j;
    /* merge into left sibling */
...
static int leaf_remove(struct bplus_tree *tree, struct bplus_leaf *leaf,
                       bp_key_t key) {
...
static int bplus_tree_delete(struct bplus_tree *tree, bp_key_t key) {
    struct bplus_node *node = tree->root;
    while (node != NULL) {
        if (is_leaf(node)) {
            struct bplus_leaf *ln = (struct bplus_leaf *)node;
...
long long bplus_tree_get(struct bplus_tree *tree, bp_key_t key) {
    long long data = bplus_tree_search(tree, key);
    // printf("bplus_tree_get: %llx\n", data);
    if (data) {
        return data;
    } else {
        return -1;
...
int bplus_tree_put(struct bplus_tree *tree, bp_key_t key, long long data) {
    if (data) {
        // printf("inserted: key=%16llx, value=%16llx\n", (long long)key, data);
        return bplus_tree_insert(tree, key, data);
    } else {
        // printf("deleted: key=%llx, value=%llx\n", (long long)key, data);
...
struct bplus_tree *bplus_tree_init(int order, int entries) {
    ...
    int i;
    // printf("! enter bplus_tree_init\n");
    struct bplus_tree *tree = calloc(1, sizeof(*tree));
    // printf("! bplus_tree *tree %d\n", tree);
    if (tree != NULL) {
        tree->root = NULL;
        tree->order = order;
...
int bplus_tree_get_range(struct bplus_tree *tree, bp_key_t key1, bp_key_t key2,
                         std::set<long long> &values) {
    long long i, data = 0;
    bp_key_t min = key1 <= key2 ? key1 : key2;
    bp_key_t max = min == key1 ? key2 : key1;
    struct bplus_node *node = tree->root;
    ...
    while (node != NULL) {
        if (is_leaf(node)) {
            ...
            while (ln != NULL && ln->key[i] <= max) {
                data = ln->data[i];
                // //printf("range search: %llx\n", data);
                if (data) values.insert(data);
                // //printf("range search: values count++\n");
...
static bp_key_t node_key(struct bplus_node *node, int i) {
    if (is_leaf(node)) {
