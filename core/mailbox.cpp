...
Mailbox::Mailbox(const Node &node, const std::vector<Node> &nodes,
...
void Mailbox::StopReceiving() {
	Barrier();
	Message exit_msg;
	exit_msg.meta.recver_node = node_.id;
	exit_msg.meta.msg_type = MSG_T::EXIT;
	Send(exit_msg);
...
void Mailbox::Receiving() {
	...
	while (true) {
		...
		if (msg.meta.msg_type == MSG_T::EXIT) {
			break;
		} else if (msg.meta.msg_type == MSG_T::BARRIER) {
			std::unique_lock<std::mutex> lk(barrier_mu_);
			barrier_count_ += 1;
			if (barrier_count_ == nodes_.size()) {
				VLOG(1) << "Collected " << nodes_.size()
				<< " barrier, Node:" << node_.id << " unblocking main thread";
				barrier_cond_.notify_one();
			}
		} else {
			CHECK(queue_map_.find(msg.meta.recver_node) != queue_map_.end());
			queue_map_[msg.meta.recver_node]->Push(std::move(msg));
		}
	}
}

// TODO: Why message sending must be sequential
int Mailbox::Send(const Message &msg) {
	std::lock_guard<std::mutex> lk(mu_);
	// find the socket
	int id = msg.meta.recver_node;
...
void Mailbox::Barrier() {
	for (auto &node : nodes_) {
		Message barrier_msg;
		barrier_msg.meta.sender_node = node_.id;
		barrier_msg.meta.recver_node = node.id;
		barrier_msg.meta.msg_type = MSG_T::BARRIER;
		Send(barrier_msg);
	}
	std::unique_lock<std::mutex> lk(barrier_mu_);
