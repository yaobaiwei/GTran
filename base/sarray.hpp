...
class SArray {
 public:
  ...
  friend ibinstream& operator<<(ibinstream& bin, const SArray<V>& sarray) {
    bin << sarray.size();
    bin.raw_bytes(reinterpret_cast<char*>(sarray.data()), sarray.size()*sizeof(V));
    return bin;
  }

  friend obinstream& operator>>(obinstream& bin, SArray<V>& sarray) {
    size_t size;
    bin >> size;
    void* ptr = bin.raw_bytes(size * sizeof(V));
