...
class RDMA {
    ...
    public:
        ...
        RDMA_Device(int num_workers, int num_threads, int nid, rdma_mem_t mem_info, vector<Node> & nodes, Node & master) : num_threads_(num_threads), nid_(nid){
            // record IPs of ndoes
            vector<string> ipset;
            ...
            int outstanding = min(UD_MAX_RECV_SIZE, (int)(mem_info.mem_dgram_recv_sz / RDMA_UD_RECV_BUF_SZ));
