...
class CPUInfoUtil {
 private:
    ...
    // info that read in
    // thread count per socket; need to be the same.
    // Will this double as the physical core cnt when HT disabled?
    // No. It has been proved that when HT disabled, siblings will equals to the core per socket.
    std::vector<int> siblings_;
    // core count per socket; need to be the same
    std::vector<int> cpu_cores_;
    // core id may not be continual. a 10-core processor may have a core id of 12
