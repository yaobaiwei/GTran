...
void VKVStore::insert_vertex_properties(vector<VProperty*> & vplist) {
    cout << "size of vplist: " << vplist.size() << endl;
...
void VKVStore::get_property_local(uint64_t pid, elem_t & elem) {
    ikey_t key;
...
void VKVStore::get_property_remote(int tid, int dst_nid, uint64_t pid, elem_t & elem) {
    ikey_t key;
