...
void ValidationActor::process(const QueryPlan & qplan, Message & msg) {
    ...
    step_aobj_map_t step_obj_map;
    process_trx(num_queries, cur_qid, trx_step_sets, step_obj_map);

    // ===================Step 3======================//
    // map<Primitive -> map<TrxID -> vector<RCTValue>>>
    unordered_map<int, unordered_map<uint64_t, vector<rct_read_data_t>>> rct_content_map;
    ...
    for (int i = 0; i < static_cast<int>(Primitive_T::COUNT); i++) {
        set<vstep_t> pre_vstep_set = primitiveStepMap[i];
    ...
    // ===================Step 5======================//
    step_trx_rct_map check_step_map;
    for (int i = 0; i < static_cast<int>(Primitive_T::COUNT); i++) {
    ...
    bool isAbort = do_step_validation(cur_trxID, check_step_map, optimistic_validation_trx, step_obj_map);
...
void ValidationActor::prepare_primitive_list() {
    ...
    needValidateActorSet.emplace(ACTOR_T::TRAVERSAL);
    needValidateActorSet.emplace(ACTOR_T::VALUES);
    needValidateActorSet.emplace(ACTOR_T::PROPERTY);
    needValidateActorSet.emplace(ACTOR_T::KEY);
    needValidateActorSet.emplace(ACTOR_T::HASLABEL);
    needValidateActorSet.emplace(ACTOR_T::HAS);
    needValidateActorSet.emplace(ACTOR_T::PROJECT);
    ...
    primitiveStepMap[static_cast<int>(Primitive_T::IV)] = step_set;
    primitiveStepMap[static_cast<int>(Primitive_T::DV)] = step_set;
    primitiveStepMap[static_cast<int>(Primitive_T::IE)] = step_set;
    primitiveStepMap[static_cast<int>(Primitive_T::DE)] = step_set;
    // I/M/D VP
    step_set.clear();
    ...
    primitiveStepMap[static_cast<int>(Primitive_T::MVP)] = step_set;
    primitiveStepMap[static_cast<int>(Primitive_T::MEP)] = step_set;

    step_set.emplace(ACTOR_T::KEY);
    step_set.emplace(ACTOR_T::HAS, Step_T::HASNOT, 0);
    step_set.emplace(ACTOR_T::HAS, Step_T::HASKEY, 0);
    primitiveStepMap[static_cast<int>(Primitive_T::IVP)] = step_set;
    primitiveStepMap[static_cast<int>(Primitive_T::DVP)] = step_set;
    step_set.emplace(ACTOR_T::TRAVERSAL);
    primitiveStepMap[static_cast<int>(Primitive_T::IEP)] = step_set;
    primitiveStepMap[static_cast<int>(Primitive_T::DEP)] = step_set;
}

void ValidationActor::process_trx(int num_queries, int cur_qid, set<vstep_t> & trx_step_sets, step_aobj_map_t& step_obj_map) {
    for (uint64_t query_index = 0; query_index < num_queries; query_index++) {
        ...
        for (auto & cur_actor_obj : c_ac->second.actors) {
            vstep_t vstep;
            step_counter++;
            if (needValidateActorSet.find(cur_actor_obj.actor_type) != needValidateActorSet.end()) {
                // Analyse the step type, only for has actor
                if (cur_actor_obj.actor_type == ACTOR_T::HAS) {
                    get_vstep_for_has(cur_actor_obj, step_counter, trx_step_sets, step_obj_map);
                } else {
                    get_vstep(cur_actor_obj, step_counter, trx_step_sets, step_obj_map);
                }
            }
        }
    }
...
void ValidationActor::get_recent_action_set(const vector<uint64_t> & trxIDList,
        unordered_map<int, unordered_map<uint64_t, vector<rct_read_data_t>>> & rct_map) {
    for (int p = 0; p < static_cast<int>(Primitive_T::COUNT); p++) {
        unordered_map<uint64_t, vector<rct_read_data_t>> trx_rct_map;
        pmt_rct_table_->GetRecentActionSet((Primitive_T)p, trxIDList, trx_rct_map);
        rct_map[p] = move(trx_rct_map);
    }
}

void ValidationActor::get_vstep(const Actor_Object & cur_actor_obj, int step_num,
        set<vstep_t> & step_sets, step_aobj_map_t & step_obj_map) {
    vstep_t vstep;
    ...
    insert_step_obj_map(step_obj_map, vstep, cur_actor_obj);
}

void ValidationActor::get_vstep_for_has(const Actor_Object & cur_actor_obj, int step_num,
        set<vstep_t> & step_sets, step_aobj_map_t & step_obj_map) {
...
bool ValidationActor::do_step_validation(int cur_trxID, step_trx_rct_map & check_step_map,
        vector<uint64_t> & optimistic_validation_trx, step_aobj_map_t & step_obj_map) {
    for (auto & each_step : check_step_map) {
        ...
        for (auto & each_rct_trx : each_step.second) {
            ...
            if (!actors_->at(static_cast<ACTOR_T>(each_step.first.actor_type))->
                    valid(cur_trxID, step_obj_map.at(each_step.first), each_rct_trx.second)) {
...
void ValidationActor::insert_step_obj_map(step_aobj_map_t & step_obj_map, const vstep_t & vstep, Actor_Object cur_actor_obj) {
    if (step_obj_map.find(vstep) != step_obj_map.end()) {
        step_obj_map.at(vstep).emplace_back(cur_actor_obj);
    } else {
        vector<Actor_Object> tmp_index_vec;
        tmp_index_vec.emplace_back(cur_actor_obj);
        step_obj_map.emplace(vstep, tmp_index_vec);
    }
}
