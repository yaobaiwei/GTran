...
public:
	...
	void process(const vector<Actor_Object> & actor_objs, Message & msg) {
		...
		printf("%f, ACTOR = %s, %s, msg.meta.step = %d, msg.meta.step = %d, node = %d, tid = %d\n", 
			node.WtimeSinceStart(), "LabelActor::process", actor_objs[msg.meta.step].DebugString().c_str(), msg.meta.step, node.get_local_rank(), tid);
